#!/usr/bin/env python3
"""
Финальная очистка файлов в smeshariki-fandom
"""
...
def clean_file(file_path: Path):
    ...
    # Если заголовка нет, создаем из имени файла
    if not title_line:
    ...
    # Берем контент после заголовка
    content_lines = lines[start_idx:]
    
    # Пропускаем служебные строки в начале
    while content_lines and (
        content_lines[0].strip() == '' or
        content_lines[0].strip() == '---' or
        '**Источник:**' in content_lines[0] or
        '**URL:**' in content_lines[0] or
...
def main():
    print("Финальная очистка файлов...")
